...
| `transition_to_vtol` | N/N/N/N | None | N/N/Y/N | Pinned ArduPlane `AUTO` `MAV_CMD_DO_VTOL_TRANSITION` with `param1=MAV_VTOL_STATE_MC`; a fresh post-ACK multicopter VTOL state, same session, armed state and stable two-second post-transition position/velocity are required. It verifies `Q_ENABLE=2`, `Q_FRAME_CLASS=7`, `Q_TILT_ENABLE=1`, `Q_TILT_MASK=3`, `Q_TILT_TYPE=0`, `Q_TILT_RATE_UP=40` and `Q_TILT_MAX=45`; the pre-transition envelope is within 40 m of the explicit recovery coordinates, within 2 m of the captured post-recovery altitude and inside the 15–25 m relative-home band, with five stable position samples spanning 2 s. An operator/test authority installs a loiter mission and establishes AUTO after recovery; because AUTO entry initially sets VTOL state, the already-qualified fixed-wing transition restores fixed-wing state before readiness is measured. NOMAD does not gain generic `set_mode` |
