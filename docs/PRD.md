...
The system follows **Clean Architecture** principles, divided into four logical domains:

1.  **Domain A: Transport Layer (Connectivity)** – The secure tunnel and routing layer.
2.  **Domain B: Edge Core (The Brain)** – The Onboard Orchestrator, Vision Engine, and VIO pipeline running on the Jetson.
3.  **Domain C: Web Client (The Eyes)** – A supervisory dashboard for situational awareness and evidence logging.
4.  **Domain D: Mission Planner Integration (The Failsafe)** – A custom plugin for telemetry injection and ELRS fallback control.
...
*Objective: Establish a zero-configuration, secure, bi-directional tunnel between the Drone and Ground Station, bypassing Carrier-Grade NAT (CGNAT).*

*   **[NOMAD-NET-001] Mesh VPN Infrastructure**
    *   The system **MUST** utilize a peer-to-peer VPN solution (**Tailscale**) to create a virtual Local Area Network (LAN).
    *   The Drone and Ground Station **MUST** have static internal IP addresses (e.g., `100.x.y.z`) to ensure seamless connectivity regardless of 4G carrier changes.
*   **[NOMAD-NET-002] Mavlink Routing**
    ...
    *   **Output 3 (Ground):** UDP `<VPN_IP>:14550` (For Mission Planner).
*   **[NOMAD-NET-003] Link Bonding & Failover**
    *   The system **SHOULD** prioritize the 4G VPN interface for high-bandwidth data (Video/Logs).
...
*   **[T1-CALC-01] Relative Description Generator:**
    ...
    *   **Logic:** Raycast to find Target GPS -> Find nearest Landmark -> Calculate relative vector.
...
### 3.3 Domain C: Web Client (Dashboard)
*Objective: A "Single Pane of Glass" for the Co-Pilot built with Vue.js/React.*

*   **[WEB-UI-01] Dual Video Stream:** The dashboard **MUST** display two low-latency (<500ms) WebRTC streams via MediaMTX:
    *   **Primary:** Switchable between ZED (Navigation) and Gimbal (Targeting).
    *   **PIP:** Secondary view.
*   **[WEB-UI-02] Task 1 Gallery:** A real-time sidebar **MUST** populate with captured snapshots. The generated text description **MUST** be editable by the Co-Pilot before final submission.
*   **[WEB-UI-03] Task 2 AR Overlay:**
    *   **Red Box:** Active Target (Purple).
    *   **Green Box:** Extinguished Target (in Exclusion Map).
    *   **Button:** "Manual Fire" override.

### 3.4 Domain D: Mission Planner Integration
*Objective: Deep integration with the industry-standard GCS for safety and fallback.*

*   **[MP-PLUG-01] ELRS Tunneling:** The C# Plugin **MUST** allow the user to send high-level commands (e.g., "Start Auto-Extinguish", "Switch to VIO Mode") via the ELRS Mavlink stream. This ensures control even if 4G/VPN fails inside the metal shed.
...
*   **[COMP-01] Uncooperative Conduct:** All network traffic **MUST** be encrypted via WireGuard (Tailscale) to prevent interference.
*   **[COMP-02] Evidence:** The Orchestrator **MUST** automatically zip all mission logs and captured images into a standardized folder structure (`/Mission_Data/Task_X/`) accessible via HTTP download immediately post-flight.
*   **[COMP-03] Safety Failsafe:** If the ZED 2i disconnects or VIO confidence drops below threshold indoors, the system **MUST** automatically switch the drone to `ALT_HOLD` mode (Barometer/IMU only) to prevent a fly-away.

---

## 6. Development Architecture Diagram Description

1.  **Transport:** `Tailscale` creates the mesh. `Mavlink-Router` splits the traffic.
2.  **Edge (Process 1 - Orchestrator):** `FastAPI` handles logic. `Pymavlink` talks to FC. `NTP` syncs time.
3.  **Edge (Process 2 - Vision):** `ZED SDK` handles VIO. `YOLOv8` handles Detection. `PID` handles Gimbal. `WebRTC` handles Streaming.
4.  **Ground:** `Web Browser` talks to API/WebRTC. `Mission Planner` talks to Mavlink-Router (UDP) and ELRS (Serial).
