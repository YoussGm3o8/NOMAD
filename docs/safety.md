...
| H-22 Recovery tolerance is mistaken for transition readiness, or ACK/stale multicopter telemetry is mistaken for completed transition | SR-TYP-01: transition-to-VTOL uses the explicit recovery coordinates, a captured post-recovery altitude inside 15–25 m above home, a stabilized 40 m envelope and authoritative post-ACK state | Copter/Plane/Unknown, stale telemetry, wrong mode/state, outside-envelope, unstable samples, ACK-only, pre-ACK state, interruption and intermediate-only cases send nothing or fail; pinned source requires AUTO, and independent state observation proves armed multicopter completion. This does not prove a safe landing or pilot handover | G-M/G7 |
